...
A country states its own facts in exactly two modules and one document. Nothing
in the shared code learns the country's name, and no existing country is edited
...
4. Add `valhalla/lua/countries/<cc>.lua`: the access classes and the carrier
   ...
   entry. Add assertions to `valhalla/lua/spec/access_spec.lua`.
5. Add `rules/src/amgraph_rules/countries/<cc>.py` and register it in
...
Steps 1 to 3 are where a country is won or lost. Steps 4 to 6 are an afternoon.
